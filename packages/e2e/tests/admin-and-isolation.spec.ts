...
import { readOwnerCredentials, uniqueEmail, uniqueSuffix } from "./fixtures/users";
...
test.describe.serial("administration and data isolation", () => {
  ...
  test("a non-admin cannot reach admin, and cannot read the owner's upload by id", async ({
    ...
    const colleagueContext = await browser.newContext({ storageState: undefined });
    try {
      ...
      const upload = await page.request.post("/api/v1/uploads", {
        multipart: { file: validPngFile(`billow-e2e-owner-${uniqueSuffix()}.png`) },
        headers: { origin },
      });
